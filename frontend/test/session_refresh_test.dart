...
import 'package:mca_project/services/api_client.dart';
import 'package:mca_project/services/session_manager.dart';
...
void main() {
  ...
  test('an authenticated call succeeds through NearzyHttp', () async {
  ...
  test('a rejected access token is refreshed and the call replayed', () async {
    final manager = SessionManager.instance;
    ...
    expect(response.statusCode, 200,
        reason: 'the 401 should have been refreshed and retried: '
            '${response.body}');
    expect(manager.active!.session.accessToken, isNot('not.a.valid.jwt'));
    expect(manager.active!.session.accessToken, isNot(original));
  }, skip: !live ? 'backend not running' : null);

  test('an expired access token is renewed before the call goes out', () async {
    final manager = SessionManager.instance;
    await manager.signIn(
      responseBody: await _login(ApiConst.customerLoginUrl, _customerEmail),
      fallbackRole: Roles.ROLE_CUSTOMER,
  ...
  test('refresh tokens rotate, and a replay ends the session', () async {
    final manager = SessionManager.instance;
    await manager.signIn(
      responseBody: await _login(ApiConst.customerLoginUrl, _customerEmail),
      fallbackRole: Roles.ROLE_CUSTOMER,
  ...
  test('concurrent calls on an expired token trigger exactly one refresh',
      () async {
    final manager = SessionManager.instance;
    await manager.signIn(
      responseBody: await _login(ApiConst.customerLoginUrl, _customerEmail),
  ...
  test('two accounts stay signed in and switching needs no password',
      () async {
    final manager = SessionManager.instance;
  ...
  test('signing out drops only that account and activates the other',
      () async {
    final manager = SessionManager.instance;
    await manager.signIn(
      responseBody: await _login(ApiConst.customerLoginUrl, _customerEmail),
  ...
  test('sign-out revokes the refresh token server-side', () async {
    final manager = SessionManager.instance;
    await manager.signIn(
      responseBody: await _login(ApiConst.customerLoginUrl, _customerEmail),
      fallbackRole: Roles.ROLE_CUSTOMER,
  ...
  test('a session restored from a pre-refresh build survives a restart',
      () async {
    ...
    final legacyBody = await _login(ApiConst.customerLoginUrl, _customerEmail);
...
Future<void> _mutateActiveSession(
    Map<String, dynamic> Function(Map<String, dynamic>) change) async {
  ...
  await SecureStorage.storeData(
      key: 'nearzy_accounts', value: jsonEncode(state));
...
Future<void> _expireStoredSession() => _mutateActiveSession((session) => {
      ...session,
